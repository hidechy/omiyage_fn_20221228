...
import 'package:family_notes/data/repository/check_sheet_growth.dart';
import 'package:family_notes/data/request/check_sheet_growth_save/request.dart';
import 'package:family_notes/provider/maintenance/notifier.dart';
...
final checkSheetGrowthProvider = AutoDisposeStateNotifierProvider<
    CheckSheetGrowthNotifier, CheckSheetGrowthState>((ref) {
  final childId = ref
...
class CheckSheetGrowthNotifier extends StateNotifier<CheckSheetGrowthState> {
  ...
  CheckSheetGrowthStateLoaded get _getLoaded =>
      state as CheckSheetGrowthStateLoaded;
  ...
  Future<void> _fetchDetails() async {
    ...
    await checkSheetGrowthRepository
        .fetchDetails(childId: childId!)
        .then((response) {
  ...
  void onChangedIsShowQuestionCategoryArea() {
    ...
    final newCategoryList = loadedState.list.map((category) {
      final checkList = category.questionList
          ...
          .toList();
      final isShowQuestionCategoryArea = !isShowOnlyUnAnswered ||
          (isShowOnlyUnAnswered && checkList.contains(questionCheckNo));

      return category.copyWith(
          isShowQuestionCategoryArea: isShowQuestionCategoryArea);
    }).toList();
  ...
  void onChangedIsShowQuestionArea() {
    ...
    final newCategoryList = loadedState.list.map((category) {
      final newQuestionList = category.questionList.map((question) {
        final isShowQuestionArea = !isShowOnlyUnAnswered ||
            (questionCheckNo == question.isCheck && isShowOnlyUnAnswered);
  ...
  void onTapCheck({
    ...
    final loadedState = _getLoaded;
    final category =
        loadedState.list.firstWhere((category) => category.id == categoryId);
    final newQuestionList = category.questionList.map((question) {
    ...
    final newCategoryList = loadedState.list.map((category) {
      return category.id == categoryId
          ? category.copyWith(questionList: newQuestionList)
          : category;
    }).toList();
