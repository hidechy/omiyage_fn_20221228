...
class PregnantHealthCheckInputNotifier
        ...
        state = PregnantHealthCheckInputPageStatus.loaded(
          data: PregnantHealthCheckInputState(
            inputData: PregnantHealthCheckInputData(
              ...
              weight: checkupDetail.weight?.toString() ?? '',
              selectedItem: {
  ...
  void onTapRegister({
    ...
    if (loadedState.data.loading) {
      return;
    }
    state = loadedState.copyWith(
      data: loadedState.data.copyWith(loading: true),
    );
    ...
    repository.saveCheckupRecord(request: request).then(
      (response) {
        state = loadedState.copyWith(
          data: loadedState.data.copyWith(loading: false),
        );
